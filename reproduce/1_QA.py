...
def get_args():
    ...
    parser.add_argument("--outputpath", type=str, default="./tests/dpsk_light/dpsk_light_output.csv")
    parser.add_argument("--workingdir", type=str, default="./tests/dpsk_light")
    ...
    parser.add_argument("--mode", type=str, default="light", choices=["naive", "light", "mini"])
    ...
    # 并发与稳定性参数
    parser.add_argument("--query_concurrency", type=int, default=12, help="外层问题并发数")
    parser.add_argument("--llm_max_async", type=int, default=48, help="MiniRAG内部LLM最大并发")
    ...
    parser.add_argument("--api_timeout", type=float, default=600.0, help="单次LLM请求超时秒数")
...
async def stats_reporter(stats: ThroughputStats, stop_event: asyncio.Event, interval_sec: int):
    while True:
        try:
...
async def aquery_one(question: str, mode: str):
    ...
    try:
        answer = await rag.aquery(question, param=build_query_param(mode))
...
async def fill_answers_concurrently(rows, result_column, mode, resume, output_path, fieldnames):
    ...
    semaphore = asyncio.Semaphore(args.query_concurrency)
    stats = ThroughputStats()
    stop_event = asyncio.Event()
    reporter_task = asyncio.create_task(stats_reporter(stats, stop_event, args.stats_interval_sec))
